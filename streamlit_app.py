...
TYPE_MAP = {
    ...
    "WEATHERHAZARD": "PERIGO CLIMATICO",
}
SUBTYPE_MAP = {
    ...
    "HAZARD_WEATHER": "CONDICOES CLIMATICAS", "HAZARD_ON_ROAD_POT_HOLE": "BURACO NA VIA",
    "HAZARD_ON_ROAD_ROAD_KILL": "ANIMAL NA VIA", "HAZARD_ON_ROAD_CAR_STOPPED": "VEICULO PARADO",
    "HAZARD_ON_ROAD_CONSTRUCTION": "OBRAS NA VIA", "HAZARD_ON_ROAD_OBJECT": "OBJETO NA VIA",
    "HAZARD_ON_ROAD_TRAFFIC_LIGHT_FAULT": "SEMAFORO QUEBRADO",
    "HAZARD_WEATHER_FOG": "NEBLINA", "HAZARD_WEATHER_HAIL": "GRANIZO",
    "HAZARD_WEATHER_HEAVY_RAIN": "CHUVA FORTE", "HAZARD_WEATHER_FLOOD": "INUNDACAO",
    "ACCIDENT_MAJOR": "ACIDENTE GRAVE", "ACCIDENT_MINOR": "ACIDENTE LEVE",
    "JAM_HEAVY_TRAFFIC": "TRANSITO PESADO", "JAM_MODERATE_TRAFFIC": "TRANSITO MODERADO",
    "JAM_STAND_STILL_TRAFFIC": "TRANSITO PARADO",
}
        ...
        if "line" in df.columns:
            def _mid(val):
                try:
                    ...
                    return None, None
            coords    = df["line"].apply(lambda x: pd.Series(_mid(x), index=["lat", "lon"]))
            df["lat"] = coords["lat"]
            df["lon"] = coords["lon"]
...
def generate_incidents_map(df_json):
    ...
    for _, row in df_map.iterrows():
        try:
            ...
            coord_str = str(round(lat_val, 4)) + ", " + str(round(lon_val, 4))
            popup_html = (
...
def generate_jams_map(df_json):
    ...
    if "lat" in df_valid.columns and not df_valid.empty:
    ...
    for _, row in df_valid.iterrows():
        try:
            ...
            lat_val   = float(row["lat"])
            lon_val   = float(row["lon"])
            spd_str   = str(round(speed_kmh)) + " km/h"
            popup_html = (
...
st.sidebar.markdown("### Status da Sessao")
...
st.sidebar.metric("Proximo ciclo",  str(minutos_restantes) + ":" + str(segundos_restantes).zfill(2))
st.sidebar.metric("Atualizacoes",   st.session_state.manual_refreshes)
...
try:
    ...
    st.markdown("""
    **Verifique:**
    - As credenciais `gcp_service_account` estao configuradas em **Settings > Secrets**
    - A Service Account tem acesso as pastas do Drive
...
selected_date = st.sidebar.date_input(
    "Data", value=default_date, min_value=min_date, max_value=max(max_date, today_foz)
)
...
filtro_rua  = st.sidebar.text_input("Buscar Rua", placeholder="Ex: Av. Brasil")
hora_range  = st.sidebar.slider("Horario", 0, 23, (0, 23))
...
if not df_alerts_raw.empty:
    ...
    if df_filtered.empty:
        st.sidebar.warning("Sem dados em " + str(selected_date) + ". Exibindo dados mais recentes.")
        df_filtered = df_alerts_raw[
            (df_alerts_raw["type"].isin(filtro_tipo)) &
            (df_alerts_raw["hour"].between(hora_range[0], hora_range[1]))
        ].copy()
...
col_f3.metric("Rua",     ("'" + filtro_rua + "'") if filtro_rua else "Todas")
col_f4.metric("Horario", str(hora_range[0]).zfill(2) + ":00 - " + str(hora_range[1]).zfill(2) + ":59")
st.markdown("---")
...
v_media_kmh      = (
    ...
    if not df_jams_filtered.empty and "speed" in df_jams_filtered.columns and df_jams_filtered["speed"].notna().any()
    else 0
)
status_via = "Critico" if incidentes_dia > 15 else ("Moderado" if incidentes_dia > 5 else "Normal")

kpi1.metric("Total Alertas", incidentes_dia)
kpi2.metric("Acidentes",     acidentes_graves)
kpi3.metric("Vel. Media",    str(round(v_media_kmh, 1)) + " km/h")
...
with col_vel:
    ...
    fig_vel.update_layout(
        title="Velocidade Media: " + str(round(v_media_kmh, 1)) + " km/h",
...
tab_inc, tab_jams, tab_calor, tab_graficos, tab_dados = st.tabs([
    "Incidentes", "Congestionamentos", "Mapa de Calor", "Graficos", "Dados Detalhados"
])

with tab_inc:
    st.caption("Centro: -25.54, -54.58 | Norte para cima | Clique nos pontos para detalhes")
    if not df_filtered.empty:
        m_inc = generate_incidents_map(df_filtered.to_json(date_format="iso"))
        if m_inc:
            st_folium(m_inc, width="100%", height=500, key="mapa_inc")
        else:
            st.info("Nenhum incidente dentro da area de Foz do Iguacu.")
    else:
        st.info("Nenhum incidente com os filtros aplicados.")

with tab_jams:
    st.caption("Jams disponiveis: " + str(len(df_jams_raw)) + " total | " + str(len(df_jams_filtered)) + " apos filtro")
    if not df_jams_filtered.empty:
        ...
        if m_jam:
            ...
            st.markdown("**Legenda:** Verde >80 km/h | Amarelo 40-80 km/h | Laranja 20-40 km/h | Vermelho <20 km/h")
        else:
            st.warning("Congestionamentos carregados, mas sem coordenadas validas na area de Foz.")
...
with tab_calor:
    st.subheader("Zonas de Concentracao de Incidentes")
    if not df_filtered.empty:
...
with tab_graficos:
    if not df_filtered.empty:
        ...
        with col_g2:
            st.subheader("Proporcao por Tipo")
...
with tab_dados:
    ...
    if not df_filtered.empty:
        ...
        st.dataframe(
            ...
            column_config={
                "timestamp":   st.column_config.DatetimeColumn("Horario (Foz)", format="DD/MM HH:mm"),
                "type":        "Tipo",
                "subtype":     "Subtipo",
                "street":      "Rua",
        ...
        st.download_button("Baixar CSV", csv, "alertas_foz.csv", "text/csv")
    else:
...
st.caption(
    "Fonte: Google Drive | Hora Foz: " + hora_foz_atual.strftime("%H:%M") + " (UTC-3) | "
    "Atualizacoes manuais: " + str(st.session_state.manual_refreshes) + " | "
    "App online ha " + str(tempo_total // 60) + " min"
)
